...
void print_yaml(const YAML::Node& node, int level)
{
    if (node.IsNull()) {
        ...
        LOG_INFO(g_logger) << std::string(level * 4, ' ') << node.Scalar() << " - " << node.Type();
    }
    else if (node.IsMap()) {
        ...
        for (size_t i = 0; i < node.size(); ++i) {
            LOG_INFO(g_logger) << std::string(level * 4, ' ') << i << " - " << node[i].Type();
            print_yaml(node[i], level + 1);
        }
    }
    else {
        LOG_INFO(g_logger) << std::string(level * 4, ' ') << "ERROR - " << node.Type();
    }
}
