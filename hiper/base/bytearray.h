...
#ifndef HIPER_BYTEARRAY_H
#define HIPER_BYTEARRAY_H
...
public:
    ...
    /**
     * @brief 写入固定 (Fixed) 长度int8_t类型的数据
     * @post position_ += sizeof(value)
     *       如果m_position > size_ 则 size_ = position_
     */
     ...
     * @param[in] buf 内存缓存指针
     * @param[in] size 数据大小
     * @post position_ += size, 如果m_position > size_ 则 size_ = position_
     */
     ...
     * @post 如果m_position > size_ 则 size_ = position_
     * @exception 如果m_position > capacity_ 则抛出 std::out_of_range
     */
     ...
     * @param[in] len 写入的长度
     * @return 返回实际的长度
     * @post 如果(position_ + len) > capacity_ 则 m_capacity扩容N个节点以容纳len长度
     */
